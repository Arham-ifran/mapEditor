...
class WFACP_Divi_Form extends WFACP_Divi_HTML_BLOCK {
	...
	private function global_typography() {
		...
		$globalSettingOptions = [
			...
			'%%order_class%% #wfacp-e-form  button#place_order',
	...
	private function collapsible_order_summary() {
		...
		$color_selector       = [
			'%%order_class%% #wfacp-e-form .wfacp_mb_mini_cart_sec_accordion_content button.button.wfacp-coupon-btn',
		];
		$hover_color_selector = [
			'%%order_class%% #wfacp-e-form .wfacp_mb_mini_cart_sec_accordion_content button.button.wfacp-coupon-btn:hover',
		];

		$control_id = $this->add_controls_tabs( $tab_id, __( 'Coupon Button Color', 'elementor' ) );
		$fields     = [];
		$fields[]   = $this->add_color( $tab_id, 'wfacp_collapsible_coupon_text_color', $color_selector, __( '', 'woofunnels-aero-checkout' ) );
		$fields[]   = $this->add_background_color( $tab_id, 'wfacp_collapsible_coupon_background_color', $color_selector, "", 'Background Color' );
		$this->add_controls_tab( $control_id, "Normal", $fields );
		$fields   = [];
		$fields[] = $this->add_color( $tab_id, 'wfacp_collapsible_coupon_text_hover_color', $color_selector, __( '', 'woofunnels-aero-checkout' ) );
		$fields[] = $this->add_background_color( $tab_id, 'wfacp_collapsible_coupon_background_hover_color', $hover_color_selector, "", 'Background Color' );
