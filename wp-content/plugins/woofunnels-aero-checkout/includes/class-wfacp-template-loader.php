...
final class WFACP_Template_loader {
	...
	public function add_default_template( $force = false ) {
		if ( true === $force || ( ( ( isset( $_REQUEST['page'] ) && 'wfacp' === $_REQUEST['page'] ) || ( isset( $_REQUEST['action'] ) && 'wfacp_import_template' === $_REQUEST['action'] ) ) && isset( $_REQUEST['wfacp_id'] ) && $_REQUEST['wfacp_id'] > 0 ) ) {  //phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			if ( ! empty( $designs ) ) {
				$licence_exist = true;
				if ( ( WFACP_Core()->importer instanceof WFACP_Template_Importer ) && ( false == WFACP_Core()->importer->get_license_key() && ( false == $this->get_license_key_funnel_builder_pro() ) ) ) {
					$licence_exist = false;
				}
	...
	public function load_template( $wfacp_id ) {
		...
		if ( 'embed_form' !== $this->current_template->get_template_type() && false === apply_filters( 'wfacp_block_editor_compatibility', false ) ) {
	...
	public function localize_page_builder_texts() {
		...
		foreach ( $get_all_opted_page_builders as $builder ) {
			...
			$plugin_string   = sprintf( __( 'This template needs <strong>%s plugin</strong> activated.', 'woofunnels-flex-funnels' ), esc_html( $page_builder['title'] ) );
			$button_text     = __( 'Activate', 'woofunnels-flex-funnels' );
			$no_install      = 'no';
			$title           = __( 'Import Template', 'woofunnels-flex-funnels' );
			$show_cancel_btn = 'yes';
			...
			$string          = sprintf( __( ' Click the button to install and activate %s.', 'woofunnels-flex-funnels' ), esc_html( $page_builder['title'] ) );
			...
			if ( 'divi' === $builder ) {
				if ( 'activated' !== $theme_status && 'activate' === $plugin_status ) {
					...
					$button_text     = __( 'Cancel', 'woofunnels-flex-funnels' );
					$show_cancel_btn = 'no';
					$no_install      = 'yes';
			...
			$pageBuildersTexts[ $builder ] = array(
				...
				'close_btn'       => __( 'Cancel', 'funnel-builder' ),
			);
		}
